...
def get_comments_by_project(session: Session, project_id: uuid.UUID, skip: int = 0, limit: int = 100) -> Sequence[Comment]:
    statement = select(Comment).where(Comment.project_id == project_id).offset(skip).limit(limit)
    return session.exec(statement).all()

def update_comment(session: Session, db_comment: Comment, comment_update: CommentUpdate) -> Comment:
