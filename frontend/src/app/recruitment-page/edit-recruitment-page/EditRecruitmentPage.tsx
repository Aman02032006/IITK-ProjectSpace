...
function EditRecruitmentPageContent() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      router.push(`/recruitment-page?id=${recruitmentId}`);
    } catch (error: unknown) {
      if (getErrorMessage(error, "") === "Unauthorized") { router.replace("/auth"); return; }
          ...
          {!loading && !error && (
            <>
              <div className="edit-page__actions">
                <button className="edit-page__action-btn" onClick={() => router.push(`/recruitment-page?id=${recruitmentId}`)} disabled={isSubmitting}>
                  ← Go Back
                </button>
