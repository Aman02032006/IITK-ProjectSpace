...
const TeamChip: React.FC<{ member: UserSummary; colorIndex: number }> = ({ member, colorIndex }) => (
  <Link href={`/profile-page?id=${member.id}`} className="project-team-chip">
  ...
  return (
    <div className="app-shell">
      <Header
        ...
        editHref={isTeamMember ? `/project-page/edit-project-page?id=${projectId}` : undefined}
                ...
                <div className="project-creator-info">
                  <div className="project-avatar-stack">
                    {(project.team_members || []).slice(0, 4).map((m, i) => (
                      <Link key={m.id} href={`/profile-page?id=${m.id}`} className={`project-avatar-stack-item c${(i % 5) + 1}`} title={m.fullname}>
